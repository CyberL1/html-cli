...
func build(this js.Value, args []js.Value) any {
	...
	for len(dirsToProcess) > 0 {
		...
		dirContents, err := afero.ReadDir(fs, currentDir)
		if err != nil {
		...
		for _, file := range dirContents {
			...
			fileContents, err := afero.ReadFile(fs, filepath.Join(currentDir, file.Name()))
			if err != nil {
				fmt.Println("Build failed:", err)
				return errorToJS(err)
			}
			...
			fs.MkdirAll(filepath.Join(constants.Config.Build.Directory, currentDir), 0775)
			afero.WriteFile(fs, filepath.Join(constants.Config.Build.Directory, currentDir, file.Name()), fileContents, 0644)
		}
	}
	fmt.Println("\nBuild complete")
	return nil
